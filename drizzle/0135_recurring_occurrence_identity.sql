...
CREATE UNIQUE INDEX IF NOT EXISTS `idx_task_recurrence_occurrences_identity` ON `task_recurrence_occurrences` (`series_id`,`rule_revision_id`,`effective_kind`,`effective_value`);--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS `idx_task_recurrence_occurrences_task` ON `task_recurrence_occurrences` (`task_id`);--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS `idx_task_recurrence_occurrences_generation` ON `task_recurrence_occurrences` (`generated_from_task_id`);--> statement-breakpoint
INSERT OR IGNORE INTO `task_recurrence_occurrences` (
  `occurrence_id`,
  `task_id`,
...
SELECT
  'occurrence:v1:{"effective":{"kind":'
    || json_quote(CASE WHEN successor.`due_date` LIKE '____-__-__T%' THEN 'instant' ELSE 'local-date' END)
    || ',"value":' || json_quote(successor.`due_date`)
    || '},"revisionId":' || json_quote(json_extract(successor.`metadata`, '$.canonicalRecurrence.revision.id'))
    || ',"seriesId":' || json_quote(json_extract(successor.`metadata`, '$.canonicalRecurrence.series.id'))
    || ',"version":1}',
  successor.`id`,
  predecessor.`id`,
  json_extract(successor.`metadata`, '$.canonicalRecurrence.series.id'),
  json_extract(successor.`metadata`, '$.canonicalRecurrence.revision.id'),
  ...
  json_extract(successor.`metadata`, '$.canonicalRecurrence.semantics.timezone.id'),
  json_extract(successor.`metadata`, '$.canonicalRecurrence.semantics.timezone.kind'),
  'on-completion',
  'mission-control',
  'mission-control',
  json_extract(successor.`metadata`, '$.canonicalRecurrence.series.identity.stableId'),
  NULL,
  NULL,
  NULL,
  NULL,
...
WHERE successor.`recurrence_generated_from_task_id` IS NOT NULL
  ...
  AND (
    successor.`due_date` GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]'
    OR successor.`due_date` GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T*Z'
  )
  AND (
    successor.`due_date` LIKE '____-__-__T%'
    OR successor.`due_date` = schedule.`scheduled_date`
  )
  AND predecessor.`completed_at` GLOB '*T*Z'
  AND json_valid(successor.`metadata`)
  AND json_type(successor.`metadata`, '$.canonicalRecurrence') = 'object'
  AND json_type(successor.`metadata`, '$.canonicalRecurrence.version') = 'integer'
  AND json_extract(successor.`metadata`, '$.canonicalRecurrence.version') = 1
  AND json_extract(successor.`metadata`, '$.canonicalRecurrence.semantics.mode') = 'completion'
  AND json_extract(successor.`metadata`, '$.canonicalRecurrence.semantics.materialization.strategy') = 'on-completion'
  AND json_extract(successor.`metadata`, '$.canonicalRecurrence.source.owner') = 'mission-control'
  AND json_extract(successor.`metadata`, '$.canonicalRecurrence.series.identity.kind') = 'mission-control'
  AND json_extract(successor.`metadata`, '$.canonicalRecurrence.series.identity.stableId') IS NOT NULL
  AND json_extract(successor.`metadata`, '$.canonicalRecurrence.series.id') GLOB 'series:v1:[0-9a-f]*'
  AND length(json_extract(successor.`metadata`, '$.canonicalRecurrence.series.id')) = 74
  AND substr(json_extract(successor.`metadata`, '$.canonicalRecurrence.series.id'), 11)
    NOT GLOB '*[^0-9a-f]*'
  AND json_extract(successor.`metadata`, '$.canonicalRecurrence.revision.id') GLOB 'revision:v1:[0-9a-f]*'
  AND length(json_extract(successor.`metadata`, '$.canonicalRecurrence.revision.id')) = 76
  AND substr(json_extract(successor.`metadata`, '$.canonicalRecurrence.revision.id'), 13)
    NOT GLOB '*[^0-9a-f]*'
  AND json_extract(successor.`metadata`, '$.canonicalRecurrence.semantics.timezone.id') IS NOT NULL
  AND json_extract(successor.`metadata`, '$.canonicalRecurrence.semantics.timezone.kind') IN ('iana', 'provider');