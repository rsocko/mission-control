...
CREATE UNIQUE INDEX IF NOT EXISTS `idx_task_recurrence_backfill_identity` ON `task_recurrence_backfill_decisions` (`series_id`,`rule_revision_id`,`effective_kind`,`effective_value`);--> statement-breakpoint
CREATE INDEX IF NOT EXISTS `idx_task_recurrence_backfill_task` ON `task_recurrence_backfill_decisions` (`task_id`);