CREATE TEMP TABLE "_duplicate_outlook_calendar_notification_ids" ON COMMIT DROP AS
WITH ranked_notifications AS (
  SELECT
    id,
    ROW_NUMBER() OVER (
      PARTITION BY connector_instance_id, metadata->>'eventId'
...
SET source_id = connector_instance_id || ':cal:' || (metadata->>'eventId')
WHERE connector_type = 'outlook-calendar'
  AND COALESCE(metadata->>'eventId', '') <> '';
