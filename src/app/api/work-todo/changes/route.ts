...
import { workTodoChangesRequestSchema } from '@/lib/connectors/work-todo/contracts';
import {
  leaseWorkTodoChanges,
  WorkTodoBridgeError,
...
export async function POST(request: Request) {
  ...
  const parsed = workTodoChangesRequestSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) {
    return NextResponse.json({
      error: 'Invalid Work To Do changes request',
      issues: parsed.error.issues,
    }, { status: 400 });
  }
  try {
    return NextResponse.json(await leaseWorkTodoChanges(parsed.data));
  } catch (error) {
