...
export function createSqliteConnectorExecutionRepositories(
  ...
  return {
    ...
    support: {
      ...
      async syncLegacyGitHubProjects(input) {
        for (const { project, taskSourceIds } of input.associations) {
          input.assertIdentityCurrent?.();
          if (input.blockedStableProjects?.has(project.number)) continue;
          const projectId = `gh-project:${input.connectorId}:${project.number}`;
          const existing = database.prepare(`
            SELECT metadata FROM hub_projects WHERE id = ?
          `).get(projectId) as { metadata: unknown } | undefined;
          const existingMetadata = parseObject(existing?.metadata);
          const digest = input.resolveIdentityDigest(
            project,
          ...
          if (existing) {
            database.prepare(`
              ...
              WHERE id = ?
            `).run(name, project.shortDescription ?? null, metadata, now, projectId);
          } else {
            database.prepare(`
          ...
          const stableTaskIds = input.stableProjectTaskIds?.get(project.number);
          const matching = input.stableProjectTaskIds
            ? (
                stableTaskIds && stableTaskIds.size > 0
                  ? database.prepare(`
                    ...
                    `).all(input.connectorId, ...stableTaskIds) as Array<{ id: string }>
                  : []
              )
            : (
                taskSourceIds.length > 0
                  ? database.prepare(`
                      SELECT id FROM tasks
                      WHERE connector_instance_id = ?
                        AND source_id IN (${taskSourceIds.map(() => '?').join(', ')})
                    `).all(input.connectorId, ...taskSourceIds) as Array<{ id: string }>
                  : []
              );
          const desired = new Set(matching.map((row) => row.id));
