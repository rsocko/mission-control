...
import { getNotificationLevelRank, normalizeNotificationLevel } from '@/lib/notifications/levels';
...
import { NOTIFICATION_CREATION_TRANSACTION } from './notification-delivery';
import type {
  CreateNotificationInput,
  CreateNotificationOptions,
  CreateNotificationResult,
  NotificationCreationPersistence,
  NotificationCreationTransactionCapability,
...
export function createSqliteStoredNotificationPushPolicyResolver(
  ...
  return {
    resolve(input: ResolveStoredNotificationPushPolicyInput): ResolvedNotificationPushPolicy {
...
interface CreationContext {
  ...
  policyResolver: ReturnType<typeof createSqliteStoredNotificationPushPolicyResolver>;
}
...
function createContext(
  ...
  return {
    ...
    policyResolver: createSqliteStoredNotificationPushPolicyResolver(database),
  };
}
...
export function createSqliteNotificationsInTransaction(
...
export function installSqliteNotificationCreationTransactionCompatibility(
  database: NotificationDatabase,
): void {
  database.transaction((transaction) => {
    const prototype = Object.getPrototypeOf(transaction) as Record<PropertyKey, unknown> | null;
    if (!prototype || typeof prototype[NOTIFICATION_CREATION_TRANSACTION] === 'function') return;
    Object.defineProperty(prototype, NOTIFICATION_CREATION_TRANSACTION, {
      configurable: true,
      value: function notificationCreationTransaction(
        this: NotificationDatabase,
        inputs: readonly CreateNotificationInput[],
        options: CreateNotificationOptions = {},
      ): CreateNotificationResult[] {
        return createSqliteNotificationsInTransaction(this, inputs, options);
      } satisfies NotificationCreationTransactionCapability,
    });
  }, { behavior: 'deferred' });
}

export function createSqliteNotificationCreation(
  database: NotificationDatabase,
): NotificationCreationPersistence {
  return {
    async createNotifications(inputs, options = {}) {
      ...
      return database.transaction((transaction) => (
        createSqliteNotificationsInTransaction(transaction, inputs, options)
      ), { behavior: 'immediate' });
    },

    async resolveCurrentGlobalPushSuppression(now, channel) {
      return resolveCurrentGlobalPushSuppression(database, now, channel);
    },

    async countPendingDeliveries() {
      return Number(database.select({ count: sql<number>`count(*)` })
        .from(notificationDeliveryEvents)
