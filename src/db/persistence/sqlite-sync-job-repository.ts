...
export function renewSyncJobLease(
  jobId: string,
  owner: string,
  attempt: number,
  leaseMs = getSyncLeaseMs(),
  ...
  const transaction = sqlite.transaction(() => {
    ...
    `).get(jobId, owner, attempt, nowIso) as { connectorId: string } | undefined;
    if (!job) return false;
...
export function isSyncJobCancellationRequested(
  jobId: string,
  owner: string,
  attempt: number,
): boolean {
  const row = sqlite.prepare(`
    ...
    FROM sync_jobs
    WHERE id = ? AND status = 'running' AND lease_owner = ? AND attempt = ?
  `).get(jobId, owner, attempt) as { cancelRequestedAt: string | null } | undefined;
...
export function completeSyncJob(
  ...
  const transaction = sqlite.transaction(() => {
    const job = sqlite.prepare(`
      ...
      FROM sync_jobs
      WHERE id = ? AND status = 'running' AND lease_owner = ? AND attempt = ?
    `).get(jobId, owner, attempt) as { connectorId: string } | undefined;
    const lockOwner = connectorSyncLeaseOwner(jobId, owner);
...
export function finalizeSuccessfulSyncJob(
  ...
  const transaction = sqlite.transaction(() => {
    ...
    const ownedJob = sqlite.prepare(`
      ...
      WHERE id = ? AND status = 'running' AND lease_owner = ? AND attempt = ?
        AND lease_expires_at > ?
    `).get(job.id, owner, job.attempt, now) as { connectorId: string } | undefined;
    ...
    const completed = sqlite.prepare(`
      ...
      SET status = 'succeeded',
          ...
          lease_expires_at = NULL
      WHERE id = ? AND status = 'running' AND lease_owner = ? AND attempt = ?
    `).run(JSON.stringify(result), now, now, job.id, owner, job.attempt);
  ...
  const transaction = sqlite.transaction(() => {
    ...
    const update = sqlite.prepare(`
      ...
      SET status = ?,
          ...
          lease_expires_at = NULL
      WHERE id = ? AND status = 'running' AND lease_owner = ? AND attempt = ?
    `).run(status, availableAt, error, status, nowIso, nowIso, job.id, owner, job.attempt);
    if (update.changes !== 1) {
      throw new Error(`Sync job ${job.id} ownership was lost before failure was recorded`);
    }
    sqlite.prepare(`
...
export function releaseSyncJob(
  jobId: string,
  owner: string,
  attempt: number,
  reason: string,
  ...
  const transaction = sqlite.transaction(() => {
    ...
    const update = sqlite.prepare(`
      ...
      SET status = 'queued',
          ...
          lease_expires_at = NULL
      WHERE id = ? AND status = 'running' AND lease_owner = ? AND attempt = ?
    `).run(now, reason, now, jobId, owner, attempt);
