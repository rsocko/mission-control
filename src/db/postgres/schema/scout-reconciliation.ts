...
export const scoutReconciliationRuns = pgTable('scout_reconciliation_runs', {
...
export const scoutReconciliationEvaluations = pgTable('scout_reconciliation_evaluations', {
  id: text('id').primaryKey(),
  ...
  evidence: jsonb('evidence').$type<ReconciliationEvidenceSummary>().notNull(),
...
export const scoutReconciliationSuggestions = pgTable('scout_reconciliation_suggestions', {
  id: text('id').primaryKey(),
  taskId: text('task_id').notNull(),
  runId: text('run_id').notNull()
    .references(() => scoutReconciliationRuns.id, { onDelete: 'cascade' }),
  ...
  policyReason: text('policy_reason').notNull(),
  payloadHash: text('payload_hash').notNull(),
  proposedEffect: jsonb('proposed_effect').$type<Record<string, unknown>>().notNull(),
  createdAt: text('created_at').notNull(),
...
export const scoutReconciliationTaskState = pgTable('scout_reconciliation_task_state', {
  taskId: text('task_id').primaryKey(),
  neverAutoComplete: boolean('never_auto_complete').notNull().default(false),
