...
export const triageItems = sqliteTable('triage_items', {
  ...
  aiCategories: text('ai_categories', { mode: 'json' }).notNull().default('[]'),
  aiSuggestedActions: text('ai_suggested_actions', { mode: 'json' }).notNull().default('[]'),
  aiRelevanceScore: integer('ai_relevance_score').notNull().default(0),
  aiUrgency: text('ai_urgency').notNull().default('evergreen'),
  rawMetadata: text('raw_metadata', { mode: 'json' }).notNull().default('{}'),
  actionsTaken: text('actions_taken', { mode: 'json' }).notNull().default('[]'),
...
export const triageActionClaims = sqliteTable('triage_action_claims', {
  id: text('id').primaryKey(),
...
export const triageContentTypes = sqliteTable('triage_content_types', {
  ...
  urlPatterns: text('url_patterns', { mode: 'json' }).notNull().default('[]'), // regex strings
  keywordHints: text('keyword_hints', { mode: 'json' }).notNull().default('[]'), // terms to match in title/desc/url
  description: text('description'),
...
export const triageSyncState = sqliteTable('triage_sync_state', {
  ...
  lastRunErrors: text('last_run_errors', { mode: 'json' }).notNull().default('[]'),
