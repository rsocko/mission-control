...
export class SqliteDurableAiRunStore {
  constructor(
...
export class SqliteDurableAiRunRepository implements DurableAiRunRepository {
  constructor(private readonly store = new SqliteDurableAiRunStore()) {}

  async createRun(input: CreateDurableAiRunInput) {
  ...
  async getInternalRun(runId: string) {
    return this.store.getInternalRun(runId);
  }

  async listInternalRunsByRoute(executionRoute: string) {
    return this.store.listInternalRunsByRoute(executionRoute);
  }

  async listRuns(filter: DurableAiRunHistoryFilter = {}) {
    return this.store.listRuns(filter);
  }

  async getEventsAfter(runId: string, cursor = 0, limit = 100) {
    return this.store.getEventsAfter(runId, cursor, limit);
  }

  async getEventIdempotencyKeys(runId: string) {
    return this.store.getEventIdempotencyKeys(runId);
  }

  async appendEvent(runId: string, input: AppendDurableAiRunEventInput) {
    return this.store.appendEvent(runId, input);
  }
  ...
  async appendEventForClaim(
    ...
    return this.store.appendEventForClaim(runId, owner, attempt, input);
  }
  ...
  async claimNextRun(
    ...
    return this.store.claimNextRun(owner, routes, leaseMs, now);
  }
  ...
  async renewLease(
    ...
    return this.store.renewLease(runId, owner, leaseMs, now);
  }

  async isCancellationRequested(runId: string, owner?: string) {
    return this.store.isCancellationRequested(runId, owner);
  }

  async requestCancellation(runId: string, now = new Date()) {
    return this.store.requestCancellation(runId, now);
  }

  async retryRun(runId: string, commandIdempotencyKey: string, now = new Date()) {
    return this.store.retryRun(runId, commandIdempotencyKey, now);
  }
  ...
  async completeRun(
    ...
    return this.store.completeRun(runId, owner, outcome, now);
  }

  async cancelRun(runId: string, owner: string, now = new Date()) {
    return this.store.cancelRun(runId, owner, now);
  }

  async timeOutRun(runId: string, owner: string, now = new Date()) {
    return this.store.timeOutRun(runId, owner, now);
  }
  ...
  async failRun(
    ...
    return this.store.failRun(runId, owner, error, options);
  }

  async expireTimedOutQueuedRuns(now = new Date()) {
    return this.store.expireTimedOutQueuedRuns(now);
  }

  async recoverExpiredRuns(now = new Date(), routes?: readonly string[]) {
    return this.store.recoverExpiredRuns(now, routes);
  }
  ...
  async setProviderSession(
    ...
    return this.store.setProviderSession(runId, provider, reference, options);
  }
  ...
  async getProviderSession(runId: string, now = new Date()) {
    return this.store.getProviderSession(runId, now);
  }
  ...
  async getProviderSessionForClaim(
    ...
    return this.store.getProviderSessionForClaim(runId, owner, attempt, now);
  }

  async revokeProviderSession(runId: string, now = new Date()) {
    return this.store.revokeProviderSession(runId, now);
  }
  ...
  async revokeProviderSessionForClaim(
    ...
    return this.store.revokeProviderSessionForClaim(runId, owner, attempt, now);
  }
  ...
  async claimCleanup(
    ...
    return this.store.claimCleanup(owner, routes, leaseMs, now);
  }
  ...
  async renewCleanupLease(
    ...
    return this.store.renewCleanupLease(runId, owner, leaseMs, now);
  }
  ...
  async finishCleanup(
    ...
    return this.store.finishCleanup(runId, owner, error, now);
  }
  ...
  async initializeExecutionState(
    ...
    return this.store.initializeExecutionState(runId, state, options);
  }
  ...
  async compareAndSetExecutionState(
    ...
    return this.store.compareAndSetExecutionState(
      runId,
      expectedRevision,
      state,
      options,
