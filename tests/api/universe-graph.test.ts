...
describe('GET /api/graph/universe', () => {
  ...
  it('passes validated dimensions, filters, and budgets to the shared service', async () => {
    ...
    const response = await GET(new Request(
      'http://localhost/api/graph/universe?dimensions=priority,tags&filterQuery=assignee%3Agraph&priorities=high&statuses=todo&sources=local&listIds=local%3Ainbox&maxNodes=250&maxEdges=400',
    ));
