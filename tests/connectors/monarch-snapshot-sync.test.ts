...
describe.sequential('FinanceSnapshotSynchronizer', () => {
  ...
  it('attributes a normalized page without sending private finance fields', async () => {
    connectorConfig.settings.cardRuleFingerprintParityProven = true;
    ...
    delete connectorConfig.settings.cardRuleFingerprintParityProven;
    ...
    expect(firstAttributionBody.items[0]!.instrumentFingerprint)
      .toMatch(/^instrument-v1:[A-Za-z0-9_-]{43}$/);
  ...
  it('sends null fingerprints and blocks card-rule attribution until parity is proven', async () => {
    vi.stubEnv('TYRION_ATTRIBUTION_FINGERPRINT_KEY', 'invented-fingerprint-key-at-least-32-characters');
    vi.stubEnv('TYRION_ATTRIBUTION_KEY_VERSION', '1');
    ...
    vi.stubGlobal('fetch', vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url.hostname !== 'tyrion-operations-ui') {
        return page([
          ...
          transaction('unproven-card-transaction'),
        ], null);
      }
      const body = JSON.parse(String(init?.body)) as {
        items: Array<Record<string, unknown> & { sourceRef: string }>;
    ...
    expect(attributionBodies[0]?.items[0]?.instrumentFingerprint).toBeNull();
    expect(sqlite.prepare(`
      SELECT assigned_kid_id AS kidId, attribution_status AS status,
             attribution_last_error_code AS failureCode
      FROM finance_transactions
      WHERE upstream_transaction_id = 'unproven-card-transaction'
    `).get()).toEqual({
      kidId: null,
      status: 'unavailable',
      failureCode: 'card_rule_fingerprint_parity_unproven',
    });
    sqlite.prepare(`
      DELETE FROM finance_attribution_audit
