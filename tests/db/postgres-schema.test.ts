...
describe('PostgreSQL schema', () => {
  it('has a table and column equivalent for every SQLite schema export', () => {
    ...
    expect(Object.keys(postgresTables)).toHaveLength(158);
  ...
  it('ships one clean PostgreSQL baseline migration', () => {
    ...
    // 158 shared tables (parity with SQLite) + 2 PostgreSQL-only search-index tables.
    expect(sql.match(/^CREATE TABLE /gm)).toHaveLength(160);
