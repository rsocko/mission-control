...
describe('PostgreSQL schema', () => {
  it('has a table and column equivalent for every SQLite schema export', () => {
    ...
    expect(Object.keys(postgresTables)).toHaveLength(153);
  ...
  it('ships one clean PostgreSQL baseline migration', () => {
    ...
    // 153 shared tables (parity with SQLite) + 2 PostgreSQL-only search-index tables.
    expect(sql.match(/^CREATE TABLE /gm)).toHaveLength(155);
