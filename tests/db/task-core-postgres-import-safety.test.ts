...
function fakePersistence(): TaskCorePersistence {
  ...
  return {
    ...
    priorityEntities: {
      ...
      createPriorityEntity: (input) => record('createPriorityEntity', {
        ...input,
      ...
      updatePriorityEntities: () => record('updatePriorityEntities', undefined),
      deletePriorityEntityAndRerank: () => record('deletePriorityEntityAndRerank', undefined),
      listPriorityEntityOptions: () => record('listPriorityEntityOptions', {
        projects: [],
        tags: [],
        sources: [],
      }),
      listPrioritySyncLog: () => record('listPrioritySyncLog', []),
    ...
    organization: {
      readTagOverview: () => record('readTagOverview', { tags: [], sourceTagSlugs: [] }),
      createHubTag: ({ id, name, slug, color }) => record('createHubTag', {
        kind: 'created' as const,
        tag: { id, name, slug, type: 'hub', color },
      }),
      updateTag: () => record('updateTag', { affectedTaskIds: [] }),
      deleteHubTag: () => record('deleteHubTag', { kind: 'missing' as const }),
      getTagConsolidationCandidates: () => record(
        'getTagConsolidationCandidates',
        { target: null, sources: [] },
      ),
      mergeTags: () => record('mergeTags', { kind: 'stale' as const }),
      unifyTags: () => record('unifyTags', { kind: 'stale' as const }),
      listTaskIdsForTag: () => record('listTaskIdsForTag', []),
      getTagPushSubject: () => record('getTagPushSubject', null),
      getTagSourceRemovalContext: () => record(
        'getTagSourceRemovalContext',
        { tag: null, tasks: [] },
      ),
      ensureBuiltInSubtaskTemplates: () => record('ensureBuiltInSubtaskTemplates', undefined),
      listSubtaskTemplates: () => record('listSubtaskTemplates', []),
      getSubtaskTemplate: () => record('getSubtaskTemplate', null),
      getSubtaskTemplateApplicationPlan: () => record(
        'getSubtaskTemplateApplicationPlan',
        null,
      ),
      createSubtaskTemplate: ({ id, template, now }) => record('createSubtaskTemplate', {
        id,
        name: template.name,
        ...
        workflowTasks: template.workflowTasks,
        icon: template.icon,
        isBuiltIn: false,
        createdAt: now,
        updatedAt: now,
      }),
      updateSubtaskTemplate: () => record('updateSubtaskTemplate', null),
      deleteSubtaskTemplate: () => record('deleteSubtaskTemplate', { kind: 'missing' as const }),
      applyWorkflowTemplate: () => record('applyWorkflowTemplate', undefined),
      applySingleTemplate: () => record(
        'applySingleTemplate',
        { kind: 'missing-parent' as const },
      ),
      getTaskMoveToListContext: () => record('getTaskMoveToListContext', null),
      finalizeTaskMoveToList: () => record('finalizeTaskMoveToList', undefined),
      getTaskMovePreviewSnapshot: () => record('getTaskMovePreviewSnapshot', null),
      readSmartScoreInputs: () => record('readSmartScoreInputs', {
        tasks: [],
        sourceRankings: [],
        taskTags: [],
        taskProjects: [],
